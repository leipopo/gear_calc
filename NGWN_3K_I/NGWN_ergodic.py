...
h_r_D = 1.5  # 齿根到齿外圆的距离
max_dr1 = 50  # 一级齿圈外径圆直径上限
max_zr1 = int((max_dr1 - h_r_D * 2 - m1 * 1.25 * 2) / m1)  # 一级齿圈最大齿数
max_dr2 = 50 # 二级齿圈外径圆直径上限
max_zr2 = int((max_dr2 - h_r_D * 2 - m2 * 1.25 * 2) / m2)  # 二级齿圈最大齿数
min_eta = 0.4  # 最小效率（确保效率不会太低）

n_input = 6000  # 输入额定转速
t_input = 2.5 # 输入额定转矩
t_input_peaks_ratio = 4  # 输入转矩峰值与额定值的比值
...
for zs in range(min_zs, max_zs + 1):
    ...
    while gp.detector_col_dec(m1, n_p, zs, zp1, 0) == True:
        for zr1 in range(zs + 2 * zp1, min(max_zr1, zs + 2 * zp1) + 3):
            if (zs + zr1) % n_p == 0:
                for zr2 in range(int((zs + zp1) * m1 / m2 + min_zp2), max_zr2 + 1):
                    zp2_base = int(zr2 - m1 / m2 * (zs + zp1))
                    for zp2 in range (zp2_base-1,zp2_base+3):
