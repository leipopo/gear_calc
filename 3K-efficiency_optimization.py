        ...
        if (zs+zr1) % n_p == 0:
            for delta_zr in range(min_delta_zr, max_delta_zr + 1):
                ...
                for delta_zp in range(min_delta_zp, max_delta_zp + 1):
                    ...
                    fm = 0.05
                    # 功率损失系数
                    psai_s_p1 = math.pi / 2 * epsilon_s_p1 * fm * (1 / zs + 1 / zp1)  # 太阳轮和一级行星
                    psai_p1_r1 = math.pi / 2 * epsilon_p1_r1 * fm * (1 / zp1 - 1 / zr1)  # 一级行星和一级齿圈
                    psai_p2_r2 = math.pi / 2 * epsilon_p2_r2 * fm * (1 / zp2 - 1 / zr2)  # 二级行星和二级齿圈
                    psai_r1_r2 = psai_p1_r1 + psai_p2_r2  # 总损失系数
                    if delta_zr >= 0:
                        eta_forward = 0.98 / (1 + abs(i / (1 + p)) * psai_r1_r2)  # zr2>zr1
                        eta_backward = 0.98 * (1 - abs(i / (1 + p) + 1) * psai_r1_r2)  # zr2>zr1
                    else:
                        eta_forward = 0.98 / (1 + abs(i / (1 + p) - 1) * psai_r1_r2)  # zr1>zr2
                        eta_backward = 0.98 * (1 - (abs(i / (1 + p)) * psai_r1_r2))  # zr1>zr2
                    if max_i > abs(i) > min_i:  # 限定速比范围
                        if eta_forward > min_eta:  # 限定效率
                            formatted_result = [round(x, 3) for x in [zs, zp1, zp2, zr1, zr2, i, eta_forward, eta_backward]]
                            results.append(formatted_result)

'''使用prettytable打印结果'''

pt = PrettyTable()
pt.field_names = ["齿数s", "齿数p1", "齿数p2", "齿数r1", "齿数r2", "传动比", "正向效率", "逆向效率"]
...
pt_sorted.field_names = ["齿数s", "齿数p1", "齿数p2", "齿数r1", "齿数r2", "传动比", "正向效率", "逆向效率"]
